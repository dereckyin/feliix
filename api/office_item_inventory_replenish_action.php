...
if($status == 1)
{
        ...
        function update_office_item_qty($request_id, $request_no, $phase, $db, $user_id)
        {
            ...
            foreach ($phase_array as $key => $value) {
                ...
                $query = "insert into office_stock_history (request_id, code, qty, action, act_1, act_2, create_id, created_at, `status`, qty_before, qty_after) values (:request_id, :code, :qty, 'Inventory Replenishment', '" . $request_no . "', '" . $action . "', :updated_id, now(), 1, " . $value['qty'] . ", " . $qty . ")";
                $stmt = $db->prepare($query);
